...
public class NumberTheory {
    ...
    4. 对于模x意义下的循环节，如果x被质因数分解为p1^k1*p2^k2*…*pm^km，
    则循环节是模每个质数的幂意义下的循环节的最小公倍数。

    对于广义斐波那契数列：
    先转化为矩阵形式A={{a,b},{1,0}}，将其对角相似化为T^-1*D*T
    求出A的特征值lambda1,lambda2，则D={{lambda1,0},{0,lambda2}}，当D^n=I时A^n=I
    则lambda1^n=lambda2^n=1，设c=a^2+4b，若c是模p的二次剩余(存在x使得x^2%p=c成立)，则n=p-1
    若c不是二次剩余，则n=(p+1)(p-1)，枚举n的因数即可
    p小于等于a^2+4b的情况会出现问题，n=(p+1)p(p-1)
     */
